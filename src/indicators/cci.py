...
def golden_cross_zero(
						...
						buy_flag=True,
						sell_flag=True,
						st_percent_max_buy = 0.1,
						st_percent_max_sell = 0.1,
	...
	for elm in finding_points.index:
		...
		if buy_flag:
			if ((CCI_Low[finding_points['index'][elm]] > CCI_Low[finding_points['index'][elm]-1]) &
				...
				signal_buy['value_min_cci'][buy_counter] = np.min(CCI_High[int(finding_points['index'][elm-1]):int(finding_points['index'][elm])])
				signal_buy['value_max_cci'][buy_counter] = np.max(CCI_High[int(finding_points['index'][elm-2]):int(finding_points['index'][elm])])
				...
				signal_buy['mean_cci'][buy_counter] = np.mean(CCI_High[int(finding_points['index'][elm] - distance_lines):int(finding_points['index'][elm])])
				
				
				if finding_points['distance'][elm] != 0:
					signal_buy['ramp_candle'][buy_counter] = (dataset[symbol]['high'][int(finding_points['index'][elm])] - np.mean(dataset[symbol]['high'][(int(finding_points['index'][elm]) - int(finding_points['distance'][elm])):int(finding_points['index'][elm])]))/finding_points['distance'][elm]
				#Calculate porfits
				...
				if (mode == 'optimize'):
					...
					if (name_stp_pr == True):
						...
						list_time = np.where(
											(dataset_1H[symbol]['time'].dt.year.to_numpy() == dataset[symbol]['time'][int(finding_points['index'][elm])].year) &
											(dataset_1H[symbol]['time'].dt.month.to_numpy() == dataset[symbol]['time'][int(finding_points['index'][elm])].month) &
											(dataset_1H[symbol]['time'].dt.day.to_numpy() == dataset[symbol]['time'][int(finding_points['index'][elm])].day) &
											(dataset_1H[symbol]['time'].dt.hour.to_numpy() == dataset[symbol]['time'][int(finding_points['index'][elm])].hour)
											)[0]
						...
						if (res_pro.empty == False):
							...
							if (
								...
								if signal_buy['diff_pr_down'][buy_counter] < st_percent_min_buy:
									signal_buy['diff_pr_down'][buy_counter] = st_percent_min_buy
									res_pro['low'][2] = dataset[symbol]['low'][int(finding_points['index'][elm])] * (1-(st_percent_min_buy/100))

								if signal_buy['diff_pr_down'][buy_counter] > st_percent_max_buy:
								...
								if signal_buy['diff_pr_top'][buy_counter] > tp_percent_buy_max:
									...
									res_pro['high'][0] = dataset[symbol]['high'][int(finding_points['index'][elm])] * (1+(tp_percent_buy_max/100))
								...
								if ((len(np.where((((dataset[symbol]['low'][int(finding_points['index'][elm]):-1]).values) <= (res_pro['low'][2])))[0])-1) > 1):
									...
									signal_buy['st_pr'][buy_counter] = signal_buy['diff_pr_down'][buy_counter]#((dataset[symbol]['low'][int(finding_points['index'][elm])] - res_pro['low'][2])/res_pro['low'][2]) * 100
								else:
...
def tester_golden_cross_zero(
	...
	if flag_trade == 'buy':
		if name_stp_pr == True:
			...
			ramp_candle_pr_buy = Find_Best_intervals(signals=signal_buy,apply_to='ramp_candle',
			 	min_tp=0.0, max_st=0, name_stp='flag_pr',alpha=alpha)
			...
			list_index_ok = np.where(
				...
				(signal_buy['ramp_candle'].to_numpy()>=-1000)&#ramp_candle_pr_buy['interval'][lower]) &
				(signal_buy['ramp_candle'].to_numpy()<=1000)#ramp_candle_pr_buy['interval'][upper])
			...
			output_buy['mean_tp_pr'] = [np.mean(signal_buy['tp_pr'][np.where(signal_buy['flag_pr'] != 'no_flag')[0]].to_numpy())]
			output_buy['mean_st_pr'] = [np.mean(signal_buy['st_pr'][np.where(signal_buy['flag_pr'] != 'no_flag')[0]].to_numpy())]
	...
	#output_buy['value_max_lower_cci_pr'] = [value_max_cci_pr_buy['interval'][lower]]
			...
			if (output_buy['sum_st_pr'][0] != 0):
				...
				if (score_sum_tp > 0):
					score_sum_tp = score_sum_tp * 90
				else:
					score_sum_tp = 0.01
			else:
				score_sum_tp = output_buy['sum_tp_pr'][0]
				if (output_buy['sum_tp_pr'][0] != 0):
					score_sum_tp = output_buy['sum_tp_pr'][0] * 100
...
def initilize_values_genetic(
	...
	'distance_lines': int((high_period_upper-int(low_period_upper/2))),
	'cross_line': 0,
	'alpha': 0.5,
	#'max_st': 0.4,
	#'max_tp': 0.6,
	...
	while i < 5:
		...
		Chromosome[i] = {
			...
			'distance_lines': int((Chromosome[i]['high_period']-Chromosome[i]['low_period'])),
	...
	while (re_counter < limit_counter):
		...
		if (Chromosome[re_counter]['high_period'] <= Chromosome[re_counter]['low_period'] * 2):
			...
			Chromosome[re_counter] = {
						...
						'distance_lines': int((Chromosome[re_counter]['high_period']-Chromosome[re_counter]['low_period'])),
...
def genetic_algo_cci_golden_cross(
	...
	if os.path.exists("Genetic_cci_output_buy/"+symbol+'.csv'):
		with open("Genetic_cci_output_buy/"+symbol+'.csv', 'r', newline='') as myfile:
			for line in csv.DictReader(myfile):
				...
				Chromosome[0]['high_period'] = int(float(chrom_get['high_period']))
				Chromosome[0]['low_period'] = int(float(chrom_get['low_period']))

				if flag_trade == 'buy':
					high_period_upper = int(Chromosome[0]['high_period']) + int(Chromosome[0]['high_period']/2)
					high_period_lower = int(Chromosome[0]['high_period']) - int(Chromosome[0]['high_period']/2)
					...
					if low_period_lower < 10:
						low_period_lower = 10

					if low_period_upper < 50:
						low_period_upper = 50

	Chromosome = initilize_values_genetic(
										high_period_upper=int(high_period_upper),
										high_period_lower=int(high_period_lower),
										low_period_upper=int(low_period_upper),
										low_period_lower=int(low_period_lower)
										)
	...
	if os.path.exists("Genetic_cci_output_buy/"+symbol+'.csv'):
		with open("Genetic_cci_output_buy/"+symbol+'.csv', 'r', newline='') as myfile:
			for line in csv.DictReader(myfile):
				...
				Chromosome[0]['high_period'] = int(float(chrom_get['high_period']))
				Chromosome[0]['low_period'] = int(float(chrom_get['low_period']))
				Chromosome[0]['distance_lines'] = int(float(chrom_get['distance_lines']))
				Chromosome[0]['cross_line'] = int(float(chrom_get['cross_line']))
	...
	if os.path.exists("Genetic_cci_output_sell/"+symbol+'.csv'):
		with open("Genetic_cci_output_sell/"+symbol+'.csv', 'r', newline='') as myfile:
			for line in csv.DictReader(myfile):
				chrom_get = line
				Chromosome[1]['high_period'] = int(float(chrom_get['high_period']))
				Chromosome[1]['low_period'] = int(float(chrom_get['low_period']))
				Chromosome[1]['distance_lines'] = int(float(chrom_get['distance_lines']))
				Chromosome[1]['cross_line'] = int(float(chrom_get['cross_line']))
		...
		while chrom_counter < len(Chromosome):
			...
			if (chorm_reset_counter > 400):
				...
				Chromosome[chrom_counter] = {
											...
											'distance_lines': int((Chromosome[chrom_counter]['high_period']-Chromosome[chrom_counter]['low_period'])),
			...
			if True:
				if flag_trade == 'buy':
					buy_data, _ = golden_cross_zero(
													...
													buy_flag=True,
													sell_flag=False,
													st_percent_max_buy=max_st_buy,
													st_percent_max_sell=1,
				...
				if bad_buy == True:
					...
					if bad_score_counter_buy < 5:
						...
						if (
							...
							low_period = randint(low_period_lower,low_period_upper)
							while Chromosome[chrom_counter]['high_period'] < low_period * 2:
								low_period = randint(low_period_lower,low_period_upper)
								if (low_period == Chromosome[chrom_counter]['low_period']): continue

							Chromosome[chrom_counter] = {
								'high_period': Chromosome[chrom_counter]['high_period'],#high_period,
								'low_period': low_period,#low_period,
								'distance_lines': int(Chromosome[chrom_counter]['high_period'] - low_period),#randint(0, 6),
								'cross_line': 0,#Chromosome[chrom_counter]['cross_line'],#randint(0, cross_line_upper_out_tester),
								'alpha': randint(1, 40)/100,
								#'max_st': max_st,
								#'max_tp': max_tp,
								'signal': None,
	...
	if flag_trade == 'buy':
		if ga_result_buy['methode'][0] != 'no_trade':
			...
			buy_data,_ = golden_cross_zero(
										...
										distance_lines=distance_lines,#ga_result_buy['distance_lines'][0],
										cross_line=cross_line,#-ga_result_buy['cross_line'][0],
										mode='optimize',
										...
										buy_flag=True,
										sell_flag=False,
										st_percent_max_buy= max_st,
										st_percent_max_sell=0,
										st_percent_min_buy = min_st,
										tp_percent_minmax_sell_min = 0,
										tp_percent_sell_max = 0,
										tp_percent_buy_min = min_tp,
										tp_percent_buy_max = max_tp,
										alpha=alfa
										)
		#*********************** Min Max Methode:
		...
		#*********************** PR Methode:
			if ga_result_buy['methode'][0] == 'pr':
				...
				value_min_cci_pr_buy = Find_Best_intervals(signals=buy_data,apply_to='value_min_cci',
			 		min_tp=0.0, max_st=0, name_stp='flag_pr',alpha=alfa)
				...
				distance_pr_buy = Find_Best_intervals(signals=buy_data,apply_to='distance',
			 		min_tp=0.0, max_st=0, name_stp='flag_pr',alpha=alfa)

				ramp_candle_pr_buy = Find_Best_intervals(signals=buy_data,apply_to='ramp_candle',
			 		min_tp=0.0, max_st=0, name_stp='flag_pr',alpha=alfa)
				...
				output_buy['max_tp'] = [round(diff_top_pr_buy['interval'][upper],2)]
				output_buy['min_tp'] = [round(diff_top_pr_buy['interval'][lower],2)]
				output_buy['distance'] = [round(distance_pr_buy['interval'][upper])]
				output_buy['ramp_candle_lower'] = [ramp_candle_pr_buy['interval'][lower]]
				output_buy['ramp_candle_upper'] = [ramp_candle_pr_buy['interval'][upper]]
	...
	if flag_trade == 'buy':
		if ga_result_buy['methode'][0] != 'no_trade':
			...
			buy_data,_ = golden_cross_zero(
										...
										buy_flag=True,
										sell_flag=False,
										st_percent_max_buy= ga_result_buy['max_st'][0],
										st_percent_max_sell=0,
