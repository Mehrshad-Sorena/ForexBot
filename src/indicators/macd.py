					...
					if (name_stp_pr == True):
						...
						if (res_pro.empty == False):
							...
							if (
								...
								if ((len(np.where(((dataset[symbol]['high'][extreme_min['index'][elm]:-1].values*0.9996) >= (res_pro['high'][0])))[0]) - 1) > 1):
									signal_buy_primary['tp_pr_index'][primary_counter] = extreme_min['index'][elm] + np.min(np.where(((dataset[symbol]['high'][extreme_min['index'][elm]:-1].values*0.9996) >= (res_pro['high'][0])))[0])
									signal_buy_primary['tp_pr'][primary_counter] = signal_buy_primary['diff_pr_top'][primary_counter]#((dataset[symbol]['high'][signal_buy_primary['tp_pr_index'][primary_counter]]*0.9996 - dataset[symbol]['high'][extreme_min['index'][elm]])/dataset[symbol]['high'][extreme_min['index'][elm]]) * 100
								else:
									signal_buy_primary['tp_pr_index'][primary_counter] = -1
									signal_buy_primary['tp_pr'][primary_counter] = 0

								if ((len(np.where((((dataset[symbol]['low'][extreme_min['index'][elm]:-1]).values) <= (res_pro['low'][2])))[0])-1) > 1):
									signal_buy_primary['st_pr_index'][primary_counter] = extreme_min['index'][elm] + np.min(np.where((((dataset[symbol]['low'][extreme_min['index'][elm]:-1]).values) <= (res_pro['low'][2])))[0])
									signal_buy_primary['st_pr'][primary_counter] = signal_buy_primary['diff_pr_down'][primary_counter]#((dataset[symbol]['low'][extreme_min['index'][elm]] - dataset[symbol]['low'][signal_buy_primary['st_pr_index'][primary_counter]])/dataset[symbol]['low'][extreme_min['index'][elm]]) * 100
								else:
...
def initilize_values_genetic(
	...
	'apply_to': 'HLCC/4',
	'alpha': 0.5,
	'num_extreme': slow_period_upper - fast_period_upper,
	'signal': None,
	'score_buy': 0,
	'score_sell': 0
	}
	...
	'apply_to': 'open',
	'alpha': 0.1,
	'num_extreme': slow_period_lower - fast_period_lower,
	#'max_tp': 0.5,
	'signal': None,
	'score_buy': 0,
	'score_sell': 0
	...
	while i < 5:
		...
		Chromosome[i] = {
			...
			'num_extreme': randint(int(fast_period_lower*0.25),(slow_period_upper-fast_period_lower)),
			#'max_tp': max_tp,
			'signal': None,
			'score_buy': 0,
			'score_sell': 0
	...
	while (re_counter < limit_counter):
		...
		if (Chromosome[re_counter]['slow_period'] <= Chromosome[re_counter]['fast_period'] * 2):
			...
			Chromosome[re_counter] = {
						...
						'apply_to': np.random.choice(apply_to_list_ga),
						'alpha': randint(1, 50)/100,
						'num_extreme': randint(int(fast_period*0.25),(slow_period-fast_period)),
						#'max_tp': max_tp,
						'signal': None,
						'score_buy': 0,
						'score_sell': 0
...
def genetic_algo_div_macd(
	...
	if flag_trade == 'sell':
		if os.path.exists(sell_path):
			...
			max_st_sell = ga_result_sell['max_st_pr'][0]
			max_tp_sell = ga_result_sell['max_tp_pr'][0]
		else:
			max_st_sell = randint(50, 80)/100
			max_tp_sell = randint(50, 80)/100
		...
		if os.path.exists("Genetic_cci_output_sell/"+symbol+'.csv'):
			with open("Genetic_cci_output_sell/"+symbol+'.csv', 'r', newline='') as myfile:
				for line in csv.DictReader(myfile):
					...
					Chromosome[0]['fast_period'] = float(chrom_get['fast_period'])
					Chromosome[0]['slow_period'] = float(chrom_get['slow_period'])
					Chromosome[0]['signal_period'] = float(chrom_get['signal_period'])
					Chromosome[0]['apply_to'] = chrom_get['apply_to']
					Chromosome[0]['alpha'] = float(chrom_get['alpha'])
		...
		while chrom_counter < len(Chromosome):
			...
			if flag_golden_cross:
				...
				Chromosome[chrom_counter] = {
					...
					'apply_to': np.random.choice(apply_to_list_ga),
					'alpha': randint(1, 50)/100,
					'num_extreme': randint(int(Chromosome[chrom_counter]['fast_period']*0.25),(Chromosome[chrom_counter]['slow_period']-Chromosome[chrom_counter]['fast_period'])),
					#'max_tp': max_tp,
					'signal': None,
					'score_buy': 0,
					'score_sell': 0
					...
					if bad_score_counter_buy < 5:
						...
						if (
							...
							Chromosome[chrom_counter] = {
										...
										'apply_to': np.random.choice(apply_to_list_ga),
										'alpha': randint(1, 50)/100,
										'num_extreme': randint(int(fast_period*0.25),(Chromosome[chrom_counter]['slow_period']-fast_period)),
										#'max_tp': max_tp,
										'signal': None,
										'score_buy': 0,
										'score_sell': 0
...
def last_signal_macd_div(
	...
	if os.path.exists(sell_path_primary):
		...
		if ga_result_sell_primary['permit'][0]:
			...
			_, _, sell_data_primary, _ = divergence_macd(
														...
														name_stp_minmax=False,
														#******** Dont Doing Not Yet ********************************
														st_percent_buy_max = ga_result_sell_primary['max_st'][0],
														st_percent_buy_min = ga_result_sell_primary['min_st'][0],
														st_percent_minmax_sell = 0.9,
														tp_percent_minmax_sell_max = 0.9,
														tp_percent_buy_max = ga_result_sell_primary['max_tp'][0],
														tp_percent_buy_min = ga_result_sell_primary['min_tp'][0],
	...
	if (
		...
		(len(dataset[symbol]['close']) - 1 - lst_idx_buy_primary) <= 1
		):
		...
		if ga_result_buy_primary['methode'][0] == 'pr':
			...
			if ga_result_buy_primary['methode'][0] == 'pr' and lst_idx_buy_primary != 0:
				...
				if (res_pro_buy_primary.empty == False):
					diff_pr_top_buy_primary = (((res_pro_buy_primary['high'][0]) - dataset[symbol]['high'][lst_idx_buy_primary])/dataset[symbol]['high'][lst_idx_buy_primary]) * 100
					diff_pr_down_buy_primary = ((dataset[symbol]['low'][lst_idx_buy_primary] - (res_pro_buy_primary['low'][2]))/dataset[symbol]['low'][lst_idx_buy_primary]) * 100
