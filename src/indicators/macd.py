					...
					if (name_stp_pr == True):
						...
						if (res_pro.empty == False):
							...
							if (
								...
								if ((len(np.where(((dataset[symbol]['high'][extreme_min['index'][elm] + num_exteremes:-1].values*0.9996) >= (res_pro['high'][0])))[0]) - 1) > 1):
								...
								if ((len(np.where((((dataset[symbol]['low'][extreme_min['index'][elm] + num_exteremes:-1]).values) <= (res_pro['low'][2])))[0])-1) > 1):
					...
					if (name_stp_pr == True):
						...
						if (res_pro.empty == False):
							...
							if (
								...
								if ((len(np.where(((dataset[symbol]['low'][extreme_max['index'][elm] + num_exteremes:-1].values * 1.0004) <= (res_pro['low'][2])))[0]) - 1) > 1):
								...
								if ((len(np.where((((dataset[symbol]['high'][extreme_max['index'][elm] + num_exteremes:-1]).values) >= (res_pro['high'][0])))[0])-1) > 1):
...
def initilize_values_genetic(
	...
	'apply_to': 'HLCC/4',
	'alpha': 0.5,
	'num_extreme': randint(10,50),#slow_period_upper - fast_period_upper,
	'signal': None,
	'score_buy': 0,
	'score_sell': 0
	}
	...
	'apply_to': 'open',
	'alpha': 0.1,
	'num_extreme': randint(10,50),#slow_period_lower - fast_period_lower,
	#'max_tp': 0.5,
	'signal': None,
	'score_buy': 0,
	'score_sell': 0
	...
	while i < 5:
		...
		Chromosome[i] = {
			...
			'num_extreme': randint(10,50),#randint(int(fast_period_lower*0.25),(slow_period_upper-fast_period_lower)),
			#'max_tp': max_tp,
			'signal': None,
			'score_buy': 0,
			'score_sell': 0
	...
	while (re_counter < limit_counter):
		...
		if (Chromosome[re_counter]['slow_period'] <= Chromosome[re_counter]['fast_period'] * 2):
			...
			Chromosome[re_counter] = {
						...
						'apply_to': np.random.choice(apply_to_list_ga),
						'alpha': randint(1, 50)/100,
						'num_extreme': randint(10,50),#randint(int(fast_period*0.25),(slow_period-fast_period)),
						#'max_tp': max_tp,
						'signal': None,
						'score_buy': 0,
						'score_sell': 0
...
def genetic_algo_div_macd(
	...
	fast_period_upper = 40
	fast_period_lower = 5

	slow_period_upper = 5000
	slow_period_lower = 1000
		...
		while chrom_counter < len(Chromosome):
			...
			if flag_golden_cross:
				...
				Chromosome[chrom_counter] = {
					...
					'apply_to': np.random.choice(apply_to_list_ga),
					'alpha': randint(1, 50)/100,
					'num_extreme': randint(10,50),#randint(int(Chromosome[chrom_counter]['fast_period']*0.25),(Chromosome[chrom_counter]['slow_period']-Chromosome[chrom_counter]['fast_period'])),
					#'max_tp': max_tp,
					'signal': None,
					'score_buy': 0,
					'score_sell': 0
					...
					if bad_score_counter_buy < 5:
						...
						if (
							...
							Chromosome[chrom_counter] = {
										...
										'apply_to': np.random.choice(apply_to_list_ga),
										'alpha': randint(1, 50)/100,
										'num_extreme': randint(10,50),#randint(int(fast_period*0.25),(Chromosome[chrom_counter]['slow_period']-fast_period)),
										#'max_tp': max_tp,
										'signal': None,
										'score_buy': 0,
										'score_sell': 0
