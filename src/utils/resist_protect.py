...
def extreme_points_ichimoko(
        high, low, close, tenkan=9, kijun=26,
        senkou=52, n_clusters=15, weight=1):
...
def extreme_points_ramp_lines(
        high, low, close, length='short',
        number_min=10, number_max=10, plot=False):
    """ Ramp Lines
    #length:
    #short
    #mid
    ...
    if plot is True:
        ax0.plot(
                x_high, lr_high.predict(
                    x_high.to_numpy().reshape(-1, 1)), "C0-", c='r')

    x1_high = high[x_high]
    y_high = lr_high.predict(x_high.to_numpy().reshape(-1, 1))
    f_high = interpld(x1_high, y_high)
   # print(f_high(y3[len(y1)-1]))

    if plot is True:
        ax0.axhline(
                y=f_high(high[high.index[0] + len(high)-1]), color='r', linestyle='-')
